...
process.on("uncaughtException", error => {
	process.reporter.error(`Uncaught exception: ${error}`);
	if (error && error.stack) process.reporter.error(error.stack);
	process.exit(1);
});

process.on("unhandledRejection", error => {
	process.reporter.error(`Promise rejection: ${error}`);
	if (error && error.stack) process.reporter.error(error.stack);
	process.exit(1);
});
