...
function compilerCallback(compiler, err, stats, lastHash, options, outputOptions) {
	...
	if (err) {
		lastHash = null;
		process.logger.error(err.stack || err);
	...
	if (outputOptions.json) {
		...
		if (stats.compilation && stats.compilation.errors.length !== 0) {
			...
			if (errors[0].name === "EntryModuleNotFoundError") {
				process.logger.error("Insufficient number of arguments or no entry found.")
				process.logger.error("Alternatively, run 'webpack(-cli) --help' for usage info.");
			}
		}

		process.logger.stats(stats, outputOptions);
...
module.exports = function webpackInstance(opts) {
	...
	if (outputOptions.help) {
		process.logger.error(outputOptions.help);
		return;
	}
	...
	try {
		...
		if (err.name === "WebpackOptionsValidationError") {
			if (outputOptions.color)
				process.logger.error(`\u001b[1m\u001b[31m${err.message}\u001b[39m\u001b[22m`);
			else process.logger.error(err.message);
	...
	console.log('\n')
	process.logger.info(`webpack ${webpack.version}`);
	...
	if (outputOptions.infoVerbosity === "verbose") {
		if (outputOptions.watch) {
			compiler.hooks.watchRun.tap("WebpackInfo", compilation => {
				...
				process.logger.info("Compilation " + compilationName + " starting…");
			});
		} else {
		...
		compiler.hooks.done.tap("WebpackInfo", compilation => {
			const compilationName = compilation.name ? compilation.name : "";
			process.logger.info("Compilation " + compilationName + " finished");
		});
	}
	...
	if (outputOptions.watch) {
		...
		if (outputOptions.infoVerbosity !== "none") process.logger.info("watching the files...");
